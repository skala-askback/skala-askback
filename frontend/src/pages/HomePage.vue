  ...
  <div class="page">
    <div class="home-head">
      <div>
        ...
        <div class="page-sub">
          {{ user.campus }} {{ user.cohort }} {{ user.classNo }}반
        </div>
      </div>
    ...
    <div class="card">
      ...
      <div class="range-row">
        <label class="range-field"
          ><span>From</span><input type="date" v-model="from"
        /></label>
        <span class="range-tilde">~</span>
        <label class="range-field"
          ><span>To</span><input type="date" v-model="to"
        /></label>
        <button class="primary-btn">조회</button>
      </div>
      <div class="stat-row">
        <div class="stat">
          <div class="stat-num c-green">{{ stats.present }}</div>
          <div class="stat-label">출석</div>
        </div>
        <div class="stat">
          <div class="stat-num c-amber">{{ stats.late }}</div>
          <div class="stat-label">지각</div>
        </div>
        <div class="stat">
          <div class="stat-num c-blue">{{ stats.out }}</div>
          <div class="stat-label">외출</div>
        </div>
        <div class="stat">
          <div class="stat-num c-red">{{ stats.absent }}</div>
          <div class="stat-label">결석</div>
        </div>
        <div class="stat">
          <div class="stat-num c-purple">{{ stats.leave }}</div>
          <div class="stat-label">공가</div>
        </div>
      </div>
      <div class="rate-bar">
...
// DB 및 백엔드 연결 확인을 위한 user 목록 조회 api
...
.rate-fill {
  height: 100%;
  background: linear-gradient(90deg, #6b35ff, #a78bfa);
}
.rate-text {
  font-size: 11px;
  color: #64748b;
